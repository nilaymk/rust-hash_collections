...
use crate::{hash_map::MapIter, hash_map_internal::{Entry, FixedSizeHashMapImpl}};
use crate::hash_map::{
    OutOfCapacityError, 
    FixedSizeHashMap,
    FixedSizeHashSet
    ...
    H: Default + Hasher,
{
    ...
    pub fn out_edge_weight(&self, to_key: &K) -> u32 {
        self._graph._hash_map.get_index_of(to_key).map_or(
            0,
            |to_index| self._node_entry._out_edges.get(&to_index).map_or(0, |v| *v)
        )
    }

    pub fn key(&self) -> &'a K {self._node_entry.key()}
    pub fn value(&self) -> &'a V {self._node_entry.value()}
}
    ...
    H: Default + Hasher,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self._inner_iter.next().map(
            |(edge_index, edge_weight)| {
                self._graph._hash_map.get_entry_at(*edge_index).map(
                    |edge_entry| {
                        (Node {_node_entry: edge_entry, _graph: self._graph}, *edge_weight)
                    }
                )
            }
        ).flatten()
...
where
    ...
    H: Default + Hasher,
{
    pub fn new() -> Self {Self{_hash_map: Default::default()}}
        ...
        for (to_key, to_value) in connections {
            ...
            if let Some(node) = self._hash_map.get_mut_entry_at(index) {
                match node._out_edges.get_mut(&to_index) {
                    ...
                    None => {let _ = node._out_edges.insert(to_index, 1);}
                }
            }
    ...
    pub fn connect_to(&mut self, k: &K, to_keys: Vec<&K>) {
        ...
        for to_key in to_keys {
            ...
            if index != to_index {
                if let Some(node) = self._hash_map.get_mut_entry_at(index) {
                    let _ = node._out_edges.insert_or(to_index, 1, |weight| *weight+=1);
                }
                if let Some(to_node) = self._hash_map.get_mut_entry_at(to_index) {
                    let _ = to_node._in_edges.insert(index, ());
                }
    ...
    pub fn node(&self, key: &K) -> Option<Node<'_, K, V, C, H>> {
        self._hash_map.get_entry_and_index_of(key).map(
            |(e, _)|
            {Node {_node_entry: e, _graph: self}}
        )
    }
}


pub type FixedSizeHashGraphMap<K, V, const C: usize> = FixedSizeHashGraphImpl<K, V, C, DefaultHasher>;